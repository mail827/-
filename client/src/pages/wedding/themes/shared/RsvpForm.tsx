...
interface RsvpFormProps {
  weddingId: string;
  onSubmit: (data: { name: string; contact: string; attendance: boolean; guestCount: number; message?: string }) => Promise<void>;
  isLoading: boolean;
...
export default function RsvpForm({ weddingId: _weddingId, onSubmit, isLoading, variant = 'classic' }: RsvpFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    await onSubmit({ name, contact, attendance, guestCount, message });
    setSubmitted(true);
  };
