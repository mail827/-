...
const themeStyles = `
  .ebr-bars{display:flex;gap:2px;align-items:center;height:16px}
  .ebr-bar{display:block;width:2px;background:#7A6F60;border-radius:1px}
  .ebr-playing .ebr-bar{animation:ebr-eq .6s ease infinite alternate}
  .ebr-bar:nth-child(1){height:8px;animation-delay:0s}
  .ebr-bar:nth-child(2){height:12px;animation-delay:.15s}
  .ebr-bar:nth-child(3){height:6px;animation-delay:.3s}
  .ebr-bar:nth-child(4){height:10px;animation-delay:.1s}
  .ebr-paused .ebr-bar{height:4px!important;animation:none}
  @keyframes ebr-eq{to{height:4px}}
  .ebr-pre{font-family:'Pretendard',-apple-system,BlinkMacSystemFont,sans-serif}
  .ebr-giant{font-size:clamp(6rem,28vw,20rem);font-weight:900;line-height:0.82;letter-spacing:-0.07em;text-transform:uppercase;margin-left:-0.05em}
  .ebr-display{font-size:clamp(2.5rem,10vw,6rem);font-weight:900;line-height:1;letter-spacing:-0.04em}
  .ebr-label{font-size:0.6rem;font-weight:900;letter-spacing:0.3em;text-transform:uppercase;opacity:0.6}
`;
...
export default function EditorialBrown({ wedding, guestbooks, onRsvpSubmit, onGuestbookSubmit, isRsvpLoading, isGuestbookLoading, guestPhotoSlot , locale}: ThemeProps) {
  ...
  return (
    <div className="min-h-screen ebr-pre" style={{ background: C.bg, overflowX: 'hidden' }}>
      ...
      {wedding.bgMusicUrl && (
        <button onClick={toggleMusic} className={`fixed top-5 right-5 z-50 w-10 h-10 rounded-full flex items-center justify-center ${isPlaying ? 'ebr-playing' : 'ebr-paused'}`}
          style={{ background: 'rgba(245,239,230,0.9)', border: `1px solid ${C.border}`, boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
          <div className="ebr-bars"><span className="ebr-bar" /><span className="ebr-bar" /><span className="ebr-bar" /><span className="ebr-bar" /></div>
        </button>
      )}

      <section className="flex flex-col justify-center" style={{ height: heroHeight, padding: '1.25rem' }}>
        <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}
          style={{ fontSize: 11, fontWeight: 800, letterSpacing: '0.5em', textTransform: 'uppercase', color: C.gold, marginBottom: 20 }}>
          Editorial No. 04 / Beige
        </motion.span>
        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.2, ease: [0.22, 1, 0.36, 1] }}
          className="ebr-giant">
          PURE
        </motion.h1>
        {(wedding.heroMedia || wedding.galleries?.[0]?.mediaUrl) && (
          <motion.div initial={{ opacity: 0, scale: 1.03 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1.2, delay: 0.3 }}
            style={{ width: '75%', height: '38vh', alignSelf: 'center', margin: '12px 0', overflow: 'hidden' }}>
        ...
        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.2, delay: 0.2, ease: [0.22, 1, 0.36, 1] }}
          className="ebr-giant">
          LOVE
        </motion.h1>
      </section>

      <section style={{ padding: '5rem 1.25rem', borderTop: `1px solid ${C.dark}` }}>
        <motion.div {...sectionAnim} style={{ marginBottom: '3.5rem' }}>
          <p className="ebr-label" style={{ marginBottom: 15 }}>Cast</p>
          <p className="ebr-display">{wedding.groomName}<br />& {wedding.brideName}</p>
        </motion.div>
        ...
        <motion.div {...delayAnim(0.3)}>
          <p className="ebr-label" style={{ marginBottom: 15 }}>Location</p>
          <p className="ebr-display">{wedding.venue}</p>
          {wedding.venueHall && <p style={{ fontWeight: 300, marginTop: 8, opacity: 0.8, fontSize: 15 }}>{wedding.venueHall}</p>}
          <p style={{ fontWeight: 300, marginTop: 6, opacity: 0.6, fontSize: 13 }}>{wedding.venueAddress}</p>
      ...
      {wedding.greeting && (
        <section style={{ padding: '4rem 1.25rem', background: C.card, borderTop: `1px solid ${C.border}` }}>
          <motion.div {...sectionAnim}>
            <p className="ebr-label" style={{ marginBottom: 16 }}>Invitation</p>
          ...
          {wedding.showParents && (
            <motion.div {...delayAnim(0.2)} style={{ marginTop: '2.5rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
              <div>
                <p className="ebr-label" style={{ marginBottom: 8 }}>Groom</p>
      ...
      {wedding.loveStoryVideo && (
        <section style={{ padding: '0 1.25rem 4rem', borderTop: `1px solid ${C.border}` }}>
          <motion.div {...sectionAnim} style={{ paddingTop: '2.5rem' }}>
            <p className="ebr-label" style={{ marginBottom: 16 }}>Film</p>
      ...
      {wedding.galleries && wedding.galleries.length > 0 && (
        <section id="gallery-section" style={{ padding: '0 0 4rem', background: C.card, borderTop: `1px solid ${C.border}` }}>
          <motion.div {...sectionAnim} style={{ padding: '2.5rem 1.25rem 1.5rem' }}>
            <p className="ebr-label">Gallery</p>
          </motion.div>
      ...
      <section id="venue-section" style={{ background: C.dark, color: C.bg, padding: '4rem 1.25rem' }}>
        <motion.div {...sectionAnim} className="text-center" style={{ marginBottom: '2rem' }}>
          ...
          <p style={{ fontSize: 18, fontWeight: 800, marginBottom: "0.25rem" }}>{formatDateLocale(wedding.weddingDate, "full", locale)} {formatTimeLocale(wedding.weddingTime, locale)}</p>
          ...
          <div className="max-w-[300px] mx-auto">
            <div className="grid grid-cols-7">
              {['S','M','T','W','T','F','S'].map((d, i) => (
                <div key={i} className="text-center" style={{ fontWeight: 600, fontSize: 10, color: i === 0 ? C.gold : 'rgba(245,239,230,0.3)', padding: '6px 0' }}>{d}</div>
              ))}
              {calendarData.weeks.flat().map((day, i) => {
                const isS = i % 7 === 0;
                const isT = day === calendarData.targetDay;
                return (
                  <div key={i} className="text-center relative" style={{ fontWeight: 400, fontSize: 13, color: !day ? 'transparent' : isS ? C.gold : 'rgba(245,239,230,0.7)', padding: '8px 0' }}>
                    {isT && <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-7 h-7 rounded-full" style={{ border: `2px solid ${C.gold}` }} />}
          ...
          {wedding.showDday && <p style={{ fontSize: 13, fontWeight: 700, letterSpacing: '0.1em', color: C.gold, marginTop: '1.25rem' }}>{getDday(wedding.weddingDate)}</p>}
        </motion.div>
      ...
      <section id="rsvp-section" style={{ background: C.bg, padding: '4rem 1.25rem', borderTop: `1px solid ${C.border}` }}>
        <motion.div {...sectionAnim}>
      ...
      {(wedding.groomAccount || wedding.brideAccount) && (
        <section id="account-section" style={{ background: C.card, padding: '4rem 1.25rem', borderTop: `1px solid ${C.border}` }}>
          <motion.div {...sectionAnim}>
            <p className="ebr-label" style={{ marginBottom: '2rem' }}>Gift</p>
          </motion.div>
          <motion.div {...delayAnim(0.15)}>
      ...
      <section id="guestbook-section" style={{ background: C.bg, padding: '4rem 1.25rem', borderTop: `1px solid ${C.border}` }}>
        <motion.div {...sectionAnim}>
        ...
        <motion.div {...sectionAnim} className="text-center">
          ...
          <div className="flex justify-center gap-4">
            {wedding.groomPhone && ( <a href={`tel:${wedding.groomPhone}`} className="text-center group">
              <div className="flex items-center justify-center transition-all duration-300 group-hover:bg-[#3E362E] group-hover:text-[#F5EFE6]" style={{ width: 48, height: 48, border: `1.5px solid ${C.dark}`, marginBottom: '0.5rem' }}><Phone className="w-4 h-4" /></div>
      ...
      <footer style={{ padding: '5rem 1.25rem', background: C.dark, color: C.bg, textAlign: 'right' }}>
        <motion.div {...sectionAnim}>
          <p style={{ fontSize: 11, letterSpacing: '0.4em', marginBottom: 20, fontWeight: 300 }}>INVITATION</p>
      ...
      <AnimatePresence>
        {galleryIndex !== null && wedding.galleries && ( <GalleryModal galleries={wedding.galleries} currentIndex={galleryIndex} onClose={() => setGalleryIndex(null)} onNavigate={setGalleryIndex} theme="EDITORIAL_BROWN" usePhotoFilter={wedding.usePhotoFilter ?? true} /> )}
      </AnimatePresence>
      <AnimatePresence>
            ...
            <div>{accounts.map((acc, i) => (
              <div key={i} style={{ padding: '1.25rem 0', borderBottom: `1px solid ${C.border}` }}>
                ...
                <div className="flex justify-between items-center" style={{ marginTop: '0.25rem' }}>
                  ...
                  <button onClick={() => onCopy(`${acc.bank} ${acc.account}`, `${title}-${i}`)} className="flex items-center gap-1 transition-all duration-300 hover:bg-[#3E362E] hover:text-[#F5EFE6]"
