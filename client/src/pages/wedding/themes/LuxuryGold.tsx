...
export default function LuxuryGold({ wedding, guestbooks, onRsvpSubmit, onGuestbookSubmit, isRsvpLoading, isGuestbookLoading }: ThemeProps) {
  ...
  <Section>
    <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-center">
      ...
      <footer className="py-8 text-center text-[#444] text-[10px] tracking-[0.3em]">청첩장 작업실</footer>
      ...
      <AnimatePresence><ShareModal isOpen={showShareModal} onClose={() => setShowShareModal(false)} onShare={handleShare} variant="dark" /></AnimatePresence>
    </div>
  );
}
