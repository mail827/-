    ...
    <div className="min-h-screen bg-white" style={{ fontFamily: "'Pretendard', sans-serif" }}>
        ...
        <section className="py-16">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="px-4">
            ...
            {(() => {
              ...
              const mainImages = remainder === 0 ? galleryImages : galleryImages.slice(0, -remainder);
              const lastRowImages = remainder === 0 ? [] : galleryImages.slice(-remainder);
              return (
                <>
                  {mainImages.length > 0 && (
                    <div className="grid grid-cols-3 gap-1">
                      {mainImages.map((img, i) => (
                        <div key={i} onClick={() => setGalleryIndex(i)} className="aspect-square cursor-pointer overflow-hidden">
                          <img src={galleryThumbUrl(img)} alt="" loading="lazy" className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" />
                        </div>
                      ))}
                    </div>
                  )}
                  {lastRowImages.length > 0 && (
                    <div className={`grid gap-1 ${mainImages.length > 0 ? 'mt-1' : ''} ${lastRowImages.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
                      {lastRowImages.map((img, i) => (
                        <div key={mainImages.length + i} onClick={() => setGalleryIndex(mainImages.length + i)} className="aspect-square cursor-pointer overflow-hidden">
                          <img src={galleryThumbUrl(img)} alt="" loading="lazy" className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" />
                        </div>
                      ))}
                    </div>
      ...
      {galleryIndex !== null && wedding.galleries && (
        <GalleryModal 
          ...
          onNavigate={setGalleryIndex} theme="MODERN_MINIMAL" usePhotoFilter={wedding.usePhotoFilter ?? true}
        />
      )}
