...
const CONCEPT_LABELS: Record<string, string> = {
  ...
  lovesick: '러브시크', silver_thread: '실버스레드', summer_tape: '서머테이프', rouge_clue: '루즈클루', in_the_mood: '화양연화', summer_film: '썸머필름', lily_choucho: '릴리슈슈', nocturnal_animals: '녹터널 애니멀즈', santorini_linen: '산토리니 리넨', age_of_innocence: '순수의 시대',
...
const CATEGORY_ORDER = [
  ...
  { key: 'FILM', label: '\ud544\ub984 \ub0b4\ub7ec\ud2f0\ube0c', ids: ['lovesick', 'silver_thread', 'summer_tape', 'rouge_clue', 'in_the_mood', 'summer_film', 'lily_choucho', 'nocturnal_animals', 'santorini_linen', 'age_of_innocence'] },
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (order.skipPayment) {
        window.location.href = `/ai-snap/studio?packId=${order.packId}`;
        return;
      }
