...
export default function TabCounseling() {
  ...
  const fetchUsers = async () => {
    ...
    if (!activeUserId && Array.isArray(data) && data[0]?.id) setActiveUserId(data[0].id);
  };
  ...
  const createSession = async () => {
    try {
      ...
      if (!s?.id) throw new Error('세션 생성 응답에 id가 없습니다.');
  ...
  const send = async (text?: string) => {
    ...
    try {
      ...
      setError('');
      setInput('');
      const r = await fetch(`${API}/admin/counseling/sessions/${activeSession.id}/messages`, {
        method: 'POST',
        headers,
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-4">
        <div className="xl:col-span-3 space-y-3">
          <div className="bg-white rounded-xl border p-3">
            ...
            <button onClick={createSession} className="mt-3 w-full text-xs py-2 rounded-lg bg-stone-900 text-white">
              새 상담 시작
            </button>
          </div>
          ...
          <div className="bg-white rounded-xl border p-3">
            ...
            <div className="space-y-2 max-h-[320px] xl:max-h-[420px] overflow-auto">
              {sessions.map((s) => (
                <button
                  key={s.id}
              ...
              {sessions.length === 0 && <p className="text-xs text-stone-400">세션이 없습니다.</p>}
            </div>
          </div>
        </div>
        ...
        <div className="xl:col-span-6 bg-white rounded-xl border p-4 flex flex-col">
          ...
          <div className="flex-1 space-y-2 overflow-auto min-h-[320px] xl:min-h-[480px]">
            {messages.map((m) => (
              <div key={m.id} className={`max-w-[90%] xl:max-w-[85%] px-3 py-2 rounded-xl text-sm ${m.role === 'user' ? 'ml-auto bg-stone-900 text-white' : 'bg-stone-100 text-stone-700'}`}>
                {m.content}
              </div>
            ))}
            {messages.length === 0 && <p className="text-xs text-stone-400">메시지가 없습니다.</p>}
          </div>
          <div className="mt-3 flex gap-2">
            ...
            <button onClick={() => send()} disabled={loading || !activeSession?.id} className="px-4 rounded-lg bg-stone-900 text-white text-sm disabled:opacity-50">{loading ? '응답 중...' : '보내기'}</button>
          </div>
