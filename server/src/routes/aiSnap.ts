...
const generate = async (snapId: string, concept: string, imageUrls: string[], mode: string) => {
  try {
    ...
    const strength = isCouple ? 0.22 : (concept.startsWith('iphone_') ? 0.22 : (CRUISE_CONCEPTS.includes(concept) ? 0.20 : 0.20));
    let urls: string[];
    if (isCouple) {
...
router.post('/free/generate', authMiddleware, async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const strength = isSelfie ? 0.22 : isCruise ? 0.20 : isCouple ? 0.22 : 0.20;
