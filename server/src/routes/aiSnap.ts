...
const NEGATIVE_PROMPT = 'distorted face, deformed nose, asymmetric eyes, blurry face, smoothed skin, plastic face, bumpy skin, uneven skin texture, cartoon face, ugly face, merged faces, elongated face, enhanced jawline, square jaw, chiseled face, narrow face';
...
const COUPLE_PROMPTS: Record<string, string> = {
  ...
  outdoor_garden: 'preserve both faces with absolute accuracy, maintain exact facial proportions face shape eye spacing nose size lip shape for both subjects, keep the exact same face, facial features, face shape, eyes, nose, lips unchanged. place the same couple in an outdoor garden wedding setting, the man in navy suit on the left, the woman in flowing white dress on the right, walking hand in hand through flower archway, golden hour sunlight, lush botanical garden, preserve original facial identity exactly, do not alter or beautify the face, photorealistic, 8k, no text no logos no watermarks ultra sharp facial details, no face distortion no face morphing no face smoothing, maintain natural skin texture pores and contours for both faces, no face elongation no jaw enhancement no face slimming',
...
const generate = async (snapId: string, concept: string, imageUrls: string[], mode: string) => {
  try {
    ...
    const strength = isCouple ? 0.22 : (CRUISE_CONCEPTS.includes(concept) ? 0.30 : 0.28);
    const urls = isCouple ? [imageUrls[0]] : imageUrls;
...
router.post('/free/generate', authMiddleware, async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const urls = mode === 'couple' ? [imageUrls[0]] : imageUrls;
    const submit = await falFetch(`${FAL_QUEUE}/fal-ai/nano-banana-pro/edit`, {
      method: 'POST',
