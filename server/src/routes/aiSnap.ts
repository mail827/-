...
const SOLO_PROMPTS: Record<string, { groom: string; bride: string }> = {
  studio_classic: {
    groom: 'keep the exact same face, facial features, face shape, eyes, nose, lips unchanged. maintain exact facial proportions face shape eye spacing nose size lip shape. place the same person in a professional wedding studio, wearing elegant black tuxedo with white shirt and black bow tie, soft warm studio lighting, ivory fabric backdrop, confident gentle gaze, preserve original facial identity exactly, do not alter or beautify the face, photorealistic, 8k, no text no logos no watermarks, no face elongation no jaw enhancement no face slimming',
    bride: 'keep the exact same face, facial features, face shape, eyes, nose, lips unchanged. maintain exact facial proportions face shape eye spacing nose size lip shape. place the same person in a beautiful bridal studio, wearing elegant white lace wedding gown, holding white rose bouquet, soft warm studio lighting, ivory fabric backdrop, gentle smile, preserve original facial identity exactly, do not alter or beautify the face, photorealistic, 8k, no text no logos no watermarks, no face elongation no jaw enhancement no face slimming',
  },
  outdoor_garden: {
...
iphone_selfie: {
    groom: 'keep the exact same face, facial features, face shape, eyes, nose, lips unchanged. maintain exact facial proportions face shape eye spacing nose size lip shape. casual iPhone selfie of the same person, high angle selfie perspective arm extended, wearing crisp white dress shirt top two buttons undone sleeves rolled to forearms no tie, fitted black trousers, silver watch, visible film grain and warm noise, subtle motion blur on hair, warm golden ambient light, slightly overexposed highlights, candid natural expression relaxed smirk, NOT a studio photo NOT professional lighting, preserve original facial identity exactly, do not alter or beautify the face, photorealistic, 8k, no text no logos no watermarks, no face elongation no jaw enhancement no face slimming',
    bride: 'keep the exact same face, facial features, face shape, eyes, nose, lips unchanged. maintain exact facial proportions face shape eye spacing nose size lip shape. casual iPhone selfie of the same person, high angle selfie perspective arm extended, wearing white slip dress with thin spaghetti straps, messy loose wave hair slightly tousled, minimal dewy makeup, small gold hoop earrings, visible film grain and warm noise, subtle motion blur on hair tips, warm golden ambient light, slightly overexposed highlights, candid natural smile, NOT a studio photo NOT professional lighting, preserve original facial identity exactly, do not alter or beautify the face, photorealistic, 8k, no text no logos no watermarks, no face elongation no jaw enhancement no face slimming',
  },
  iphone_mirror: {
...
const COUPLE_PROMPTS: Record<string, string> = {
  studio_classic: 'preserve both faces with absolute accuracy, maintain exact facial proportions face shape eye spacing nose size lip shape for both subjects, keep the exact same face, facial features, face shape, eyes, nose, lips unchanged. place the same couple in a professional wedding studio, the man wearing black tuxedo with bow tie on the left, the woman wearing elegant white lace wedding gown holding bouquet on the right, couple looking at each other lovingly, soft warm studio lighting, ivory fabric backdrop, preserve original facial identity exactly, do not alter or beautify the face, photorealistic, 8k, no text no logos no watermarks ultra sharp facial details, no face distortion no face morphing no face smoothing, maintain natural skin texture pores and contours for both faces, no face elongation no jaw enhancement no face slimming',
...
iphone_selfie: 'preserve both faces with absolute accuracy, maintain exact facial proportions face shape eye spacing nose size lip shape for both subjects, keep the exact same face, facial features, face shape, eyes, nose, lips unchanged. casual iPhone couple selfie, high angle selfie perspective arm extended, the man in white dress shirt top buttons undone sleeves rolled no tie on the left, the woman in white slip dress with spaghetti straps on the right, both with candid natural laughing expressions, heads close together, visible film grain warm noise, subtle motion blur, warm golden light, slightly overexposed, NOT studio NOT professional, authentic couple selfie, preserve original facial identity exactly, do not alter or beautify the face, photorealistic, 8k, no text no logos no watermarks ultra sharp facial details, no face distortion no face morphing no face smoothing, maintain natural skin texture pores and contours for both faces, no face elongation no jaw enhancement no face slimming',
...
const generate = async (snapId: string, concept: string, imageUrls: string[], mode: string) => {
  try {
    ...
    const strength = isCouple ? 0.15 : (CRUISE_CONCEPTS.includes(concept) ? 0.30 : 0.28);
    let urls: string[];
    if (isCouple) {
...
router.post('/free/generate', authMiddleware, async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const submit = await falFetch(`${FAL_QUEUE}/fal-ai/nano-banana-pro/edit`, {
      method: 'POST',
      body: JSON.stringify({ prompt, image_urls: urls }),
    });
