...
const DYNAMIC_CONCEPTS = new Set(['retro_hongkong', 'vintage_record', 'cruise_sunset', 'cruise_bluesky', 'black_swan', 'blue_hour', 'water_memory', 'velvet_rouge', 'rose_garden', 'grass_rain', 'eternal_blue', 'heart_editorial', 'vintage_tungsten', 'aao']);
    ...
    try {
      if (pack.preferredEngine === 'seedream' && ARK_API_KEY) {
        ...
        if (effectiveMode === 'couple') {
          ...
          sdPrompt = 'Portrait 3:4 ratio. The same couple from the reference image. The man ' + gOutfit + '. The woman ' + bOutfit + '. ' + sceneBase + '. Warm friendly natural expressions with genuine gentle smiles and kind soft eyes, relaxed comfortable intimate mood, NOT stern NOT cold NOT serious NOT expressionless NOT frowning. Photorealistic editorial photograph, soft natural skin texture with visible pores, warm natural skin tone, 50mm lens, shallow depth of field, no AI artifacts no plastic skin no uncanny valley';
